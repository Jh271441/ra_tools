...
两种方式二选一：
  ...
  方式 B（无 voyager 时的兜底）：用本目录的 planning_stub_pb2.py（手写 stub）
    ...
    python3 -m grpc_tools.protoc -I scripts --python_out=scripts scripts/planning_stub.proto

用法：
  python3 scripts/read_bag.py /tmp/my_road.bag
  python3 scripts/read_bag.py /tmp/my_road.bag --topic /planning/assist_request --show-all
  python3 scripts/read_bag.py /tmp/my_road.bag --only-opened   # 只打印 is_opened=True 的行
"""
        ...
        if not os.path.exists(pb2_file):
            ...
            if not os.path.exists(proto_src):
                raise FileNotFoundError(
                    ...
                    f"请确保 planning_stub.proto 在 scripts/ 下，或能访问 {_VOYAGER_PROTO_PB}"
                )
            import subprocess
