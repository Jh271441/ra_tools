...
class RunComparisonRouteContractTest(unittest.TestCase):
    def test_page_and_api_are_public_read_only_surfaces(self) -> None:
        page_start = CORE_ROUTER.index('@router.get("/run-comparison"')
        page_block = CORE_ROUTER[page_start : page_start + 500]
        self.assertNotIn("_admin_identity", page_block)
        self.assertIn("read-only Run comparison workspace", page_block)
