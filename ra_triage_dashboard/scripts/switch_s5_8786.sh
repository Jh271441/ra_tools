...
S4_FAB_CORE_SHA=3f666d7a016a2bff1087126569ee4c32740aee1c4a530a350d773c3b0e287067
...
printf '%s' "$S5_STATUS" | python3 -c 'import json,sys; d=json.load(sys.stdin); assert int(d.get("database",{}).get("migration_count",0))==49; assert not d.get("batch_prediction_enabled"); assert not d.get("autotriage_push_enabled"); assert not (d.get("review_notifications",{}).get("enabled"))'
