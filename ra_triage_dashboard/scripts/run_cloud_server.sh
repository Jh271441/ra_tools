...
# Independent product media: meta_path is relative to this layout root (not bags/).
export ARES_CAPTURE_RA_ROOT="${ARES_CAPTURE_RA_ROOT:-${_MEDIA_LAYOUT}}"
...
# Trail probes are background-only (never block HTTP startup / first paint).
# GT labels are served from local DB + baseline xlsx; Trail refresh is optional.
export DASHBOARD_SYNC_TRAIL_ON_START="${DASHBOARD_SYNC_TRAIL_ON_START:-true}"
export DASHBOARD_GT_SYNC_STARTUP_DELAY_SECONDS="${DASHBOARD_GT_SYNC_STARTUP_DELAY_SECONDS:-60}"
...
# Review @mention notifications are an external side effect and stay fail-closed
# until a DChat application, message:send scope and BotUser credential file are
# provisioned and a gray deployment has passed.
export DASHBOARD_DCHAT_NOTIFICATIONS_ENABLED="${DASHBOARD_DCHAT_NOTIFICATIONS_ENABLED:-false}"
export DASHBOARD_DCHAT_DELIVERY_MODE="${DASHBOARD_DCHAT_DELIVERY_MODE:-openapi}"
export DASHBOARD_DCHAT_BASE_URL="${DASHBOARD_DCHAT_BASE_URL:-https://oapi-dichat.intra.xiaojukeji.com}"
export DASHBOARD_DCHAT_CREDENTIALS_FILE="${DASHBOARD_DCHAT_CREDENTIALS_FILE:-$DASHBOARD_DATA_DIR/dchat_credentials.json}"
export DASHBOARD_DCHAT_TIMEOUT_SECONDS="${DASHBOARD_DCHAT_TIMEOUT_SECONDS:-3.0}"
export DASHBOARD_DCHAT_MAX_ATTEMPTS="${DASHBOARD_DCHAT_MAX_ATTEMPTS:-5}"
