...
- 判错复核、原因聚类明细的“@讨论”和问题排除的逐 Issue 说明都支持 `@ldap` 与 `@{ldap}`。候选人只来自数据库 `mention_users` 目录，最多提及 10 人；后端会再次校验，目录外或已停用人员不能被 @。原因聚类讨论会保留当前 Run 的期望输出、Tags、证据和排除状态并追加一个 Review 版本；问题排除会在写 Trail 前先校验全部 @，回读成功后才把本地 Review 与通知 outbox 同事务落库。只有服务端验证过的 SSO 操作才会真正触发 DChat；直接 IP/LCA 的未验证姓名不能绕过目录，也不会外发通知。后台串行发送、限制在 5 QPS 内并对临时失败指数退避，DChat 不可用不会阻塞 Review 保存。
