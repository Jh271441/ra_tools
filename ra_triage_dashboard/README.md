...
| 历史抽检标签 | 0206 / 0626 XLSX 只读加载为同 baseline 的 Issue 标签预填；不是 GT，也不会自动创建 Review |
...
以及 Camera/BEV 图片资产都由服务端要求已验证且具有独立“标注权限”的账号。
标注权限分为“创建与分配 / 仅标注 / 仅查看”：三档均可查看意图页、实验记录和
标注汇总；前两档可提交标签与评论；只有“创建与分配”可创建或关闭实验。管理员
仍是主动解盲和全量导出的唯一权限边界。旧有授权用户默认保持“创建与分配”，可在
用户管理中单独调整。侧边栏隐藏仅用于界面提示，不是权限边界。数据集由版本化的 `config/intent_datasets.json` 注册，当前按
...
标注模型由批量预填的 Routing 值（左转、右转、直行、掉头、泊车）和自车变道
值（非变道、左变道、右变道），加稀疏的单时间点修改组成。历史二分类值“变道”只作为
兼容旧数据保留，不再出现在新标注按钮中。右侧用相邻的批量预填与当前帧两张卡片
...
每位标注人拥有独立的最新 revision head；活动实验只展示自己的标注，管理员可对当前
Case 显式解盲，实验关闭后则正常解盲。任务分配支持从数据集中抽取任意 Case 子集、选择
1～全部成员；交叉盲标可配置抽中 Case 的总复核人数，未抽中的 Case 仍由一名基础负责人标注。
页面默认处于“未分配（全部 Issue）”范围，不会因为当前 SSO 身份而悄悄缩窄队列；可通过
“我的任务”、实验或任务负责人筛选主动缩小范围。当前 SSO 用户仍会标记“我”；实验和负责人
筛选复用带全选、清除及“仅此项”的共享下拉
组件。每个数据集/Case 另有共享讨论区，但不会把隐藏答案自动带入评论。时间轴保持横向，
并把普通鼠标滚轮转换为横向滚动；当前帧完成 Routing 与变道两项标注并自动保存后会切到
下一帧（批量预填和多帧操作不会抢走当前焦点），当前帧会自动居中。
左侧导航使用醒目的父组标题：“判错复核”包含 Review、分析、问题排除、模型结果、
Run 对比和批次预测；“意图标注”包含标注、实验分配和独立的 `/intent-summary`
汇总页；“系统管理”包含系统状态和用户管理。意图功能不再显示“内测/Beta”标记，
是否可见仅由服务端标注权限决定。汇总页按数据集、实验、多人负责人和结果维度筛选当前
标注头，只展示至少包含一项真实标签的记录；Routing 或变道意图可以单独查看，每页支持
10/20/50 条，Case 链接可直接回到标注页。汇总不会加载 Camera/BEV；活动盲标默认
只展示当前用户答案，管理员必须显式点击解盲才能查看他人答案。
“已有标注”在解盲后按当前时间点计算每位可见标注人的有效 Routing/变道结果：该帧的
override 优先，否则回退到该人的批量预填，并以醒目标签展示；同时显示每个 Case 的帧级分布统计（例如
“直行 8 帧 · 掉头 24 帧”）。盲标进行中他人只显示“已标注 / 待标注”，不暴露具体答案。
它按内容自然增高并由整条右栏滚动，不裁剪人员记录。具有标注权限的用户可以在自己的标签右侧
悬停删除当前标注头；不可变历史版本、单帧覆盖和独立删除审计仍保留，其他标注人的头
不受影响。汇总页展示已标注帧的分布，不再使用 Issue 级默认值或“轨迹修改”计数。实验创建/关闭及标注保存都会推进共享修订号，其他在线标注页会实时刷新实验与
负责人筛选，但不会替换当前未保存的标签或重新加载大图。
`1–7` 设置当前选中帧标签、`Shift+1–7` 批量预填；`←` / `→` 切帧，
`Shift+←/→` 连续扩选，`Ctrl/⌘+←/→` 单选跳到边界，
`Ctrl/⌘+Shift+←/→` 从锚点扩选到时间轴边界，`↑` / `↓` 切 Issue。
页面级快捷键不会因为按钮或链接获得焦点而暂停；输入框、下拉框、可编辑区域与弹窗中的
快捷键保护保持不变。`Ctrl/⌘+Z` 撤销当前 Case 最近一次批量或逐帧标注操作，撤销结果
继续走同一套追加式自动保存，不删除历史版本。
`Space` 打开大图并在同一时间点的
Camera / BEV 间轮换，`B` / `C` 可直接打开对应媒体，`D` 打开讨论。输入控件、IME 和重复按键期间均不
触发。顶栏可打开可滚动并支持按键筛选的快捷键指南，`H` 可直接呼出或关闭，`R` 可刷新当前页。管理员在汇总页可直接导出
...
- 首页是服务端筛选的紧凑 Issue 缩略图队列（宽屏五列、随可用宽度降列），默认每页 20 条并可切换 10 / 20 / 50 / 100；页码和单页数量写入 URL，接口单页最多返回 100 条。判错复核与原因聚类统一显示“当前页 / 总页数”和显式跳转输入框、按钮；只有按回车或点击跳转按钮才执行，输入框失焦不会误跳页。BEV 缩略图按源文件版本生成 640×360 缓存并懒加载，不把 1071 张原图一次送进浏览器。点击 Issue 后先进入 URL 可恢复的详情态并加载 Issue、模型输出和人工 Review；BEV / Camera / 视频通过独立媒体请求在后台补齐，因此冷的共享盘扫描不会阻塞复核表单。详情支持返回列表及跨页上一/下一 Issue，并在具备 trip 与事件时间戳时提供同域 Ares Studio ±10 秒跳转链接。Issue 详情第二行通过紧凑下拉框切换 `BEV 图片 / Camera 图片 / Ares Studio 视频`，相邻的同尺寸按钮展开完整预览；有视频时详情默认展示视频并在每次切换 Issue 后定位到 t0（40 秒视频为播放器 20 秒处），没有视频时再展示 t0 或最近的 BEV / Camera 图片。Gallery 卡片的“媒体预览”和详情媒体共用一个近乎占满浏览器视口的三模式弹窗，首页仅在点击预览时按需读取该 Issue 的完整资产。详情页媒体快捷键采用页面级监听：焦点不在输入、选择、按钮或链接时，`B/C/V`、空格、左右方向键和 `F` 无需聚焦播放器即可生效；打开弹窗后由弹窗接管。判错复核页同时提供顶栏快捷键入口，按 `H` 可呼出或关闭包含 Issue 导航、标签组、媒体切换与大图控制的页面级指南。三种媒体都默认适配可视范围，并支持 1:1 原始像素、按钮/键盘/Ctrl/⌘+滚轮缩放、放大后指针拖拽平移以及全屏；进入或退出全屏不会重置缩放比例。BEV 视频使用 Workbench 自有的紧凑控制条，支持播放/暂停、0.5× / 1× / 1.5× / 2×、回到 t0、进度拖动、可配置 0.1 / 0.5 / 1 / 5 秒左右跳转和键盘控制；默认左右跳转 1 秒，元数据帧步长作为“1 帧”选项，切换到图片时暂停但保留播放位置。
- 首页的“模型判断结果”是下拉筛选：选择模型 Run 后可切换 `全部`、红色 `MISMATCH`、绿色 `MATCH`、黄色 `NO GT（GT 待复核）` 和灰色 `NONE（未预测）`；卡片右上角同步显示状态。`NO GT` 仅表示模型已有受支持输出但当前 GT 为空，不写入或伪造 GT。旧 `failure=1` / `failure_only=true` 仍兼容为 `MISMATCH`，新 Review URL 使用 `comparison=all|mismatch|match|no_gt|none`。
- 管理员“均分当前筛选”只覆盖当前 Review 队列，并保留每次分配的历史审计。任务负责人下拉框的人员与数量严格按当前 baseline、Model Run、判断结果、GT、Review 状态等筛选重新统计；其他数据集或旧 split 的负责人不会混入当前计数，移除本轮人员也不会破坏其历史分配记录。多人 Review 可设置交叉盲标比例：全部 Issue 先分配一名基础负责人，只有抽中的比例部分再增加第 2～N 名复核人；旧请求未传比例时保持 100% 交叉盲标。
- 管理员可从侧边栏进入独立的 `/review-assignments` 任务分配管理页，查看每次均分批次、整体与个人完成进度及任务明细；明细支持按负责人、待处理/已完成和 Issue 搜索。未提交的任务可转派给其他 Dashboard 用户，已提交 Review 自动锁定，所有转派保留操作者、时间和前后负责人审计记录。
...
- 左侧导航按“工作台 / 分析工具 / 意图标注 / 系统管理”分组，默认收起低频分组；分组展开状态按浏览器保存，导航栏独立滚动，当前页面所在分组会自动展开。桌面窄侧栏保留图标入口，移动端在抽屉内复用同一层级。
...
- Trail 操作分成「检查字段」和「创建 Run」两步，收在 Runs 页的默认折叠区；当前模型字段快照明确只针对 registry 的默认数据集（现为 0508/1071），不跟随顶栏多选切换。两步都不写回 Trail，快照只创建或复用本地不可变 Run，且不会修改团队默认 Run。启动时若启用 Trail 检查，也只执行第一步。问题排除页按选定 Model Run 与数据集汇总普通 Review 和已提交的盲标 Split 最新版本；多人同一 Issue 以最后追加版本为唯一排除候选，不会因普通 Review 或其他盲标版本覆盖当前结果。
...
- 判错复核首页与原因聚类页共用「Issue GT Review状态」三项筛选：待补充、与 GT 一致、GT 需复核；首页的 Gallery、URL、分页、预测当前筛选和均分任务使用同一筛选范围。任务分配页的当前批次提供“去判错复核”和“去原因聚类”入口，自动恢复创建批次时的 Run、数据集和模型判断条件，并用 `work_split` 精确限定该任务批次。判错复核不显示额外的数据范围控件；原因聚类的“Review 数据范围”会主动列出当前 Run/数据集下仍有效的任务批次，可在综合结果与任一批新增间切换。指定批次时 Gallery 保留全部任务 Issue，而原因聚类及 CSV/XLSX/GT 更新表只消费该 Split 新提交的 Review，`0/N` 不用旧结果填充，`1/N` 作为未完成结果。原因聚类页用紧凑三段比例条展示当前筛选范围内三种状态的数量和占比；宽屏时它与压缩后的 GT × 模型预测混淆矩阵左右并排，窄屏再恢复上下排列。三段状态条与图例在 hover 或键盘聚焦时联动，混淆矩阵当前格也会同步强调对应 GT 行和模型列并弱化无关格；两处仅高亮，不会隐式改变筛选。页面只消费每个 Issue 最新一版 Review：多人任务先保留每位复核人各自的最新版本计算未完成/一致/冲突，再以其中最后保存的一版作为页面、聚类与导出的完整主记录，原因、Tags、缺失信息、复核人和时间不得跨版本混合。稳定的 `missing_evidence[]` 是主聚类，Review 结构化 Tags 按「场景 / 触发判定 / 如何脱困」分别筛选，原因卡片标题保持顶部、图表主体在卡片剩余空间垂直居中；下拉筛选选择后立即刷新，搜索输入短暂防抖后立即刷新，不再需要额外点击“应用”；自由文本只作为明细中的解释，不再生成并展示旧的关键词主题卡片。自定义缺失信息按共享目录的正式标题展示，不泄露 `custom:` 或哈希 key。检索只匹配最新人工 Review 的原因、复核人、标签、Issue GT Review状态与缺失信息，不检索模型说明或 Issue 场景文本。选择 Model Run 后，“模型预测”可按三分类标签及 Stage1 `真实卡住` 筛选，另可按红色 `MISMATCH`、绿色 `MATCH`、黄色 `NO GT（模型有输出但 GT 为空）`、灰色 `NONE（未预测）` 或全部切片；`MISMATCH + MATCH + NO GT` 精确表示当前 Run 的全部受支持模型输出。混淆矩阵仍只统计有三分类 GT 的可比较 Case；明细行同时展示 GT、模型结论和人工期望输出。当前筛选结果可导出 UTF-8 CSV 或明细 XLSX；“导出 GT 更新表”额外生成张扬批量回刷工具“期望输出模式”可直接导入的两列表，只包含按“期望输出与 GT 不同”规则判定为需复核的 Issue，不信任历史手选状态。筛选、聚类和分页都写入 `/review-analysis` URL，可硬刷新并用浏览器前进/后退恢复。
- 原因聚类页提供独立的 Issue 精确筛选，支持单个或多个 Issue ID（逗号、空格、换行或 Voyager Issue 链接）；该条件以规范化后的 `issue_ids` 写入 URL，并在统计、聚类、明细以及 CSV/XLSX/GT 更新表导出之前生效，可与 Review 原因及其他筛选组合。
- Review 分析 CSV/XLSX 会保留 `应该排除` 状态；Tags 同时导出汇总、场景/触发判定/脱困/其他分组、原始 key，缺失信息同时导出标题与原始 key，并附带 Review Run / 盲标 Split 来源。`GT 更新表` 继续保持可直接导入的两列格式。
- Review Gallery 在没有显式选择复核人或任务分配人的情况下，会优先投影当前已验证登录人在该盲标任务中自己提交的最新版本；自己尚未提交时才回退到已有的 base/cross 结果。显式复核人和任务分配人筛选仍优先于当前登录人。详情编辑表单继续只绑定当前登录人自己的最新版本。
- 原因分析在 `run=none` 的全局人审视图中，活动多人任务的“问题详情”深链仍会携带该任务的实际 Model Run；进入详情后才能识别 blind split 并把保存写回正确的任务命名空间，不会静默创建普通 Review。
...
- 判错复核首页的检索框旁提供「多 Issue」精确查询弹窗。输入支持逗号、中文标点、空格、换行和 Voyager Issue 链接；系统会去重、显示无法识别的 token，并将已确认列表写入 `issue_ids` URL 参数。应用精确列表后会清空普通关键词检索；清空输入并点击查询即可恢复当前 baseline 的默认队列。该筛选只改变读取范围，不创建 Run、不修改 GT。均分任务弹窗可把当前筛选按单人或每 Issue 2～N 人盲标分配，并可选择 0%～100% 的交叉盲标比例；比例部分只有一名基础负责人之外的复核人，未抽中的 Issue 仍只分配基础负责人。人数、比例与人员使用项目 `ui-select`，不会弹出系统原生菜单。多人模式默认复用意图实验的确定性 base/cross 均衡算法，也允许为部分成员填写固定任务量、其余成员自动均衡；服务端仍保证任务总量正确且同一 Case 的复核人不重复。结果按人员展示并可复制、筛选。盲标 Review 绑定 split，但双盲只隔离每位复核人的写入版本，不限制读取：任何详情访问者都能在默认关闭的 Review 历史中查看该 Issue 的完整审计记录并据此复核其他人的结论；编辑表单始终只读取当前登录人在当前 split 的盲标版本，页面不额外显示盲标任务提示条。共享评论线程在盲标期间仍可使用。原因分析页及 CSV/XLSX/GT 更新表使用同一当前投影：当前多人 split 只要已有一人提交就优先于旧单人 Review，并允许按未完成、一致、冲突筛选；完全无人提交时才回退旧单人结果。Trail 在线属性写入仍遵守其独立的预览、确认与回读安全边界。

- Review 详情中的 `Q/W/A/S/Z/X` 标签组快捷键和 `D/J/M` 弹窗快捷键均为 toggle：首次按键展开，再按同键收起。按 `E` 进入原因编辑前会收起所有标签与缺失信息选项卡，但不会清除已选内容。
- 首页可把当前单个 Issue 或不超过 50 条的完整筛选结果预填到 Batch 页面；若前端只加载到部分结果或超过上限，会明确阻止而不是静默截断。Batch 页直接选择服务端 Provider，模型列表隐藏仅用于服务端解析的 `Auto` 别名，并保留 Profile 已验证项和网关当前在线的 Qwen3 生成模型，排除 Embedding；若 Profile 默认模型暂时离线，目录会回退到在线可用模型而不清空整个列表；实验模型有明确标记且创建任务前需再次确认。默认 Camera 输入与 `stuck_triage_auto_opt_api` 对齐为 -19s 至 +19s 的单前视 9 帧，Ares Animation 可切换且只使用服务端固定的 API 默认 manifest/时间点，浏览器不能提交路径或 Ares Capture 配置。
...
`DASHBOARD_SSO_WRITE_USERS` 与 `DASHBOARD_TEAM_DEFAULT_MANAGERS` 只在权限表为空时执行一次初始化。此后 PostgreSQL/SQLite 中的权限表是运行时权威来源：未列出的 SSO 用户与裸 IP 访问均为只读，`writer` 可执行普通 Review、Run 与 Tag 选择操作，`admin` 额外拥有独立的 `/users` 用户管理页和 `/intent-labeling` 意图标注页。`/run-comparison` 及对应 API 是面向所有看板访问者的只读分析能力，不会修改 Review、Run、GT 或 Trail；意图标注的页面路由与全部对应 API 仍会在服务端重新验证管理员身份，前端隐藏入口不作为权限边界。Kylin Portal 的「SSO 白名单」属于网关访问策略，不能直接等同于本应用写白名单。

Run 查看/对比的两侧 Run 使用项目 `ui-select`；用户可清空任一侧进入单 Run 查看，页面会收起未选侧以及变化类指标。用户选择任一侧 Run 时复用其 `inferred_baseline_ids` 自动切换顶部数据集，最后一次主动选择的 Run 决定数据集范围，不从 Run 名称猜测。普通重绘保留用户主动选择（包括零覆盖 Run），只有数据集切换恢复遗留零覆盖选择时才执行覆盖修复。

Run 对比列表仅对真实保存的 Reason 提供完整文本悬浮提示；`未保存／未生成 Reason` 是普通灰色占位，不使用问号光标，也不创建无意义的 tooltip。

Run 查看/对比始终在当前选中的不可变 GT 数据集上计算。Run 选择器会显示每个 Run 对当前数据集的覆盖数；只选择一侧时，准确率、混淆矩阵、Case 输出、Reason、Prompt 和输入配置仅展示该 Run，URL 也只保留对应的 Run 参数。双 Run 模式下，如果切换数据集后 URL 遗留的两次 Run 都是零覆盖，页面会优先改选当前数据集有输出的 Run，避免返回一个看似失败的空比较。仍允许“有覆盖 Run + 零覆盖 Run”的组合，用于查看单侧 `NONE` 的并集差异。`P/F` 表示该 Run 输出相对 GT 是否正确：`P2F` 是退化、`F2P` 是改善；比较集合取两个 Run 有效输出的并集，只要任一侧存在受支持输出就保留 Case，缺失或不支持的一侧归一为 `NONE`，仅两侧都为 `NONE` 时从明细、矩阵、统计和分页中排除。页面提供双 Run 混淆矩阵、并排的两次输出与 reason；GT 与模型输出使用无框的中性文字，只有匹配/不匹配 GT 状态使用独立的绿/红状态框，输出卡片底色保持中性，避免标签类别颜色干扰正确性判断。变化框用红→绿、纯绿、绿→红、纯红分别表达 `F2P`、`P2P`、`P2F`、`F2F`。列表 reason 保留三行预览并可 hover 查看全文；点击 Case 整行的非链接区域（或聚焦行后按 Enter/Space）会打开只读详情弹窗，单 Run 模式只展示所选 Run，双 Run 模式并排展示两次结果。Issue 链接和可见侧复核按钮保留原导航行为。页面支持按 GT、当前可见输出以及 Issue ID/reason 组合筛选；双 Run 模式额外支持变化类型和标签变化筛选。Case 默认严格按 Issue ID 升序、每页 10 条，可携带 `issue + run + comparison=all` 深链到判错复核。网页 Batch Run 若保存过 Prompt、模型及输入配置，会展示经过敏感字段脱敏的创建时快照；查看与比较本身不会修改 Review、Run、GT 或 Trail。

展开 Run 对比的参考 BEV/Camera 时，如果保存的 Run 输入含有明确 offset 的逐帧 Routing / 自车变道结果，媒体顶部会随当前帧精确显示对应值。双 Run 同值时合并展示，不同时标明基线和新 Run；没有精确 offset 映射时隐藏，不使用总计数或邻近帧猜测。
...
模型结果文件最少需要 `issue_id` 和下列任一模型输出列：

```text
model_label | stage1_prediction | ra_stuck_auto_result
```

Stage1 的 `非误触发` / `真实卡住` 会统一为 `真实卡住`，并兼容三分类 GT 的 `正确触发` / `无需协助`；`误触发` 仍只匹配 `误触发`。可选 `ra_stuck_auto_result_info`（JSON）、`model_reason` / `reason`、`model_confidence` / `confidence`、`trip_id`。原生 `run_batch.py` 结果包可直接上传：
...
「Trail 属性更新」有两个相互独立的“预览 → 明确提交” Tab：
...
- **Issue ID 屏蔽**按行接受人工输入的 Issue ID 与排除说明；一次预览/提交可以给不同 Issue 填写不同说明。该 Tab 也提供并列的 JSON / Excel 导入预览：Excel 接受 `.xlsx` / `.xlsm`，必填 `issue_id`（也支持 `Issue ID` / `问题ID`）和 `是否排除`（也支持 `should_exclude` / `is_excluded`），可选 `comment` / `排除说明` / `备注`。只有明确为“是”/`true`/`1` 的行才能替换到屏蔽草稿；“否”以及历史抽检表中常见的空白标记行都会保留在预览中显示为跳过，格式错误或重复行会阻止替换，绝不静默丢弃。Excel 文件名、工作表、行号和 SHA-256 会追加到写入 comment，便于 Trail 与上传历史追溯。JSON 兼容旧的下载草稿（未带 `是否排除` 时按“是”预览并提示），历史抽检来源会在最终屏蔽预览时重新校验。两类导入均只解析/预览，替换草稿后仍须单独生成最终预览并明确提交。每行只 deep-merge `ra_stuck_auto_result_info.ra_triage_dashboard.should_exclude=true` 与该行的 `should_exclude_comment`，保留已有模型 label；这是 Dashboard 自己的排除标记，不等同于张扬工具的原生黑名单字段，是否被其它工具消费需另行确认。排除说明与标记在同一个 info JSON 更新中写入，不调用独立评论接口。未知、重复或格式错误的 ID 不会被静默当成成功。
- 编辑区的“清空所有行”只会清空当前浏览器内的屏蔽草稿，并保留一行空输入；它会使旧预览失效，但不会请求或写入 Trail。
...
Review Tab 进入页面或切换数据集/Run 后自动加载本地汇总，不再要求点击“生成预览”；随后只调用一次紧凑状态接口，以单个批量 Trail 查询补齐每个候选的同步状态和提交能力，既不重建 Review 聚合也不阻塞表格。相同候选集在短 TTL 内复用结果，顶栏刷新才主动绕过缓存。Review 表格支持按关键词、GT、模型预测、复核人、Issue GT Review 状态和 Trail 更新状态在浏览器本地筛选，并支持 10/20/50/100 条分页；筛选条件和页码会保存到 URL。它们仅改变查看范围：状态批量查询、JSON 预览和提交始终针对完整候选草稿，不会因当前筛选而悄悄遗漏写入项。页面只把数据集作为 Run 下方的轻量范围信息，不重复渲染顶栏数据集选择器。预览中的 Payload SHA-256 是由完整候选草稿、字段和 Issue 草稿计算出的内部指纹，用于服务端拒绝过期或被修改的提交，不是 Trail 字段；Review 页面默认不展示这串内部摘要，Issue ID 预览仍可查看短指纹用于排障。未来启用写入后，提交前仍会在服务端锁内重新读取完整字段能力和目标字段，随后生成/校验摘要并逐 Issue 回读。Issue ID Tab 仍要求显式生成屏蔽预览。操作摘要 `operation_id` 与排除说明均保存在同一 `ra_triage_dashboard` info 命名空间中，重试会按 JSON 字段幂等校验。
...
1. 在左侧打开「Trail 属性更新」；Review Tab 默认汇总全部 Run，也可选择一个 Model Run；Issue ID Tab 按行填写 Issue ID 和对应的排除说明，可点击「添加一行」继续录入。
...
接口：详情首屏使用 `GET /api/cases/{issue_id}?include_media=false`，后台媒体使用 `GET /api/cases/{issue_id}/media`；Review 排除汇总使用 `GET /api/trail-attribute-update/preview?baselines=0508&probe_trail=false`（可选 `model_run_id=<run-id>`）、紧凑状态读取 `GET /api/trail-attribute-update/status?issue_ids=...` 与 `POST /api/trail-attribute-update/commit`；Issue ID 的导入预览使用 `POST /api/trail-attribute-update/issue-import/json-preview` 或 multipart `POST /api/trail-attribute-update/issue-import/excel-preview`，最终屏蔽使用 `POST /api/trail-attribute-update/issue-preview`、`POST /api/trail-attribute-update/issue-commit`，历史使用 `GET /api/trail-attribute-update/issue-history?limit=20&offset=0`。两个提交 body 都必须包含 `confirm=true` 与 `payload_sha256`；Issue ID 新客户端提交 `entries=[{"issue_id":"cn…","comment":"…"}]`，旧客户端仍可使用 `issue_ids` 和共享 `comment`。所有预览接口不调用写入 API；提交接口会重新生成最终预览并拒绝过期摘要。真实字段接口由 `ra_auto_triage/utils/trail_api.py::TrailInterface.update_issue_with_changes` 调用 `/paladin/issue/pool/multi_update/`；每行排除说明作为对应 Issue 的 `ra_stuck_auto_result_info.ra_triage_dashboard.should_exclude_comment` deep-merge 子字段写入，不调用 Comment API。
...
/review
  ?run=<model_run_id>
  &comparison=all|mismatch|match|no_gt|none
...
/review-analysis
  ...
  &issue_ids=<comma-separated_issue_ids>
  &q=<search>
  &page=<positive_integer>
```
...
26. `039_review_blind_overlap_ratio.sql`：为 Review 多人任务保存交叉盲标比例，兼容历史 split 的 100% 默认值。
