...
值（变道、非变道），加稀疏的单时间点修改组成。右侧用一块紧凑编辑器同时展示
Routing 与变道意图的批量预填和当前帧标签；批量预填会立即显示为全部时间点的最终标签，
同时保留已人工单帧修改，`0` 将当前帧两种意图都恢复为批量预填。每次保存写入追加式
revision snapshot，并使用 `expected_revision_id` 做乐观并发控制，冲突返回 `409`。
`1–7` 设置单帧标签、`Shift+1–7` 批量预填；`←` / `→` 切帧，`↑` / `↓` 切 Issue。
`Space` 打开大图并在同一时间点的
