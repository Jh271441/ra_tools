...
“当前帧已有标注”按当前时间点计算每位可见标注人的有效 Routing/变道结果：该帧的
override 优先，否则回退到该人的批量预填；同时显示每个 Case 的帧级分布统计（例如
“直行 8 帧 · 掉头 24 帧”）。它按内容自然增高并由整条右栏滚动，不裁剪人员记录。具有标注权限的用户可以
删除自己的当前标注头；不可变历史版本、单帧覆盖和独立删除审计仍保留，其他标注人的头
不受影响。实验创建/关闭及标注保存都会推进共享修订号，其他在线标注页会实时刷新实验与
