...
Run 对比的两侧 Run 使用项目 `ui-select`；用户选择任一侧 Run 时复用其 `inferred_baseline_ids` 自动切换顶部数据集，最后一次主动选择的 Run 决定数据集范围，不从 Run 名称猜测。普通重绘保留用户主动选择（包括零覆盖 Run），只有数据集切换恢复遗留双零覆盖选择时才执行覆盖修复。
