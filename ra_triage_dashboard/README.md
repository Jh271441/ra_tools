...
- Review 原因支持 `@ldap` 与 `@{ldap}`；页面只用当前 reviewer/任务负责人作为快捷候选，不提供全公司身份枚举。最多提及 10 人，保存时把 mentions 与 Review 同事务写入，并为每个收件人创建唯一 outbox 任务。只有服务端验证过的 SSO Review 才能触发 DChat；直接 IP/LCA 的未验证姓名只保留在历史中。后台串行发送、限制在 5 QPS 内并对临时失败指数退避，DChat 不可用不会阻塞 Review 保存。
...
- DChat BotUser 凭据：`/volume/home/workspace/ra_triage_dashboard_data/dchat_credentials.json`（服务用户持有的 `0600` JSON，不进入源码、环境变量、浏览器、数据库或日志）
...
需要在 DChat 开放平台创建一个应用，申请 `message:send` 权限，并为应用配置 BotUser。服务端按官方契约调用国内地址的 `POST /v3/message.create`，使用 Basic Auth、`X-Bot-Type: bot_user`、`X-Bot-Id`，并以 `receive_id_type=2` 向 LDAP 用户发送 Markdown。应用凭据文件格式如下；不要提交到 Git：
...
```bash
chmod 600 /volume/home/workspace/ra_triage_dashboard_data/dchat_credentials.json
export DASHBOARD_DCHAT_NOTIFICATIONS_ENABLED=true
export DASHBOARD_DCHAT_CREDENTIALS_FILE=/volume/home/workspace/ra_triage_dashboard_data/dchat_credentials.json
```
