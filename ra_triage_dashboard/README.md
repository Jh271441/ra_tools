...
- Trail 操作分成「检查字段可见性」和「创建只读 Trail 快照」两步；两步都不写回 Trail，快照只创建或复用本地不可变 Run，且不会修改团队默认 Run。启动时若启用 Trail 检查，也只执行第一步。
- 可导入 issue / GT 与批量模型输出（JSON、CSV、XLSX）；模型文件按 SHA-256 创建不可变 Model Run。Runs 页面默认展示团队全部 Runs，可按相关人员和来源类型筛选，并切换当前 Review Run。
...
Runs 的「相关人员」筛选同时匹配创建人和实验作者；Review 提供复核人筛选，单 case 任务历史提供请求人及任务状态筛选。旧数据没有可靠创建人时显示为历史未记录。
...
`003_identity_attribution.sql` 对旧行使用 `legacy` / `verified=false`，不会把历史自由填写姓名升级成可信 SSO。所有人工标注、模型结果与任务记录都保留历史行，因此迁移时是一次数据复制，不需要把旧记录扁平化或覆盖。上述 SQL 仍只是 PostgreSQL 目标 schema 与迁移准备；当前运行 adapter 仍是 SQLite。
