...
“已有标注”按内容自然增高并由整条右栏滚动，不裁剪人员记录。具有标注权限的用户可以
...
鼠标点击时间轴帧卡后会立即释放按钮焦点，因此上述 Ctrl/⌘、Shift 和方向键组合可
继续直接使用；输入框、下拉框与弹窗中的快捷键保护保持不变。
`Space` 打开大图并在同一时间点的
Camera / BEV 间轮换，`B` / `C` 可直接打开对应媒体，`D` 打开讨论。输入控件、IME 和重复按键期间均不
触发。顶栏可打开可滚动并支持按键筛选的快捷键指南，`H` 可直接呼出或关闭，`R` 可刷新当前页。导出接口支持
