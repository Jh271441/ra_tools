...
值（变道、非变道），加稀疏的单时间点修改组成。右侧编辑器将 Routing 与变道意图
拆成两个模式，每次只展示一种标签；批量预填会立即显示为全部时间点的最终标签，同时
保留已人工单帧修改，`0` 只将当前模式的当前帧恢复为批量预填，不会清除另一模式的修改。
每次保存写入追加式 revision snapshot，并使用 `expected_revision_id` 做乐观并发控制，冲突
返回 `409`。当前模式使用 `1–5` 设置单帧标签、`Shift+1–5` 批量预填（变道模式使用其中
的 `1–2`）；`←` / `→` 切帧，`↑` / `↓` 切 Issue。`Space` 打开大图并在同一时间点的
