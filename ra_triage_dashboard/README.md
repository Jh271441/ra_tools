...
- 人工标注为追加式历史，最新一条为当前标注，不覆盖旧 review；「模型为什么判错」是主输入，「模型缺失信息」收进紧凑的结构化多选下拉，并自动汇总为 routing、绕行空间、灯态、双闪、时序等错误聚类。每个新版本必须记录复核人及其可信状态，右侧 Review 历史默认展开并明确展示每版复核人；每个版本可粘贴或选择最多 4 张补充截图，场景 Tags 为可选的规范化多选项。
