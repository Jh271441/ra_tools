...
class DatabaseNotificationsMixin:
    ...
    def claim_intent_comment_notification(self, *, now: str) -> dict[str, Any] | None:
        """Atomically claim one due intent-comment notification."""

        with self._write_lock, self.connect() as conn:
            row = conn.execute(
                """
                SELECT id FROM intent_comment_notifications
                WHERE status IN ('pending', 'retry') AND next_attempt_at <= ?
                ORDER BY next_attempt_at ASC, id ASC
                LIMIT 1
                """,
            ...
            cursor = conn.execute(
                """
                UPDATE intent_comment_notifications
                SET status = 'sending', attempt_count = attempt_count + 1,
                    updated_at = ?
                WHERE id = ? AND status IN ('pending', 'retry')
                  AND next_attempt_at <= ?
            ...
            claimed = conn.execute(
                """
                SELECT n.*, comment.body, comment.mentions_json,
                       comment.author, comment.reply_to_id,
                       parent.author AS reply_to_author
                FROM intent_comment_notifications n
                JOIN intent_case_comments comment ON comment.id = n.comment_id
                LEFT JOIN intent_case_comments parent ON parent.id = comment.reply_to_id
                WHERE n.id = ?
                """,
                (notification_id,),
            ).fetchone()
    ...
    def claim_review_notification(self, *, now: str) -> dict[str, Any] | None:
        ...
        with self._write_lock, self.connect() as conn:
            ...
            claimed = conn.execute(
                """
                SELECT rn.*, ann.model_run_id, ann.note, ann.mentions_json,
                       ann.author
    ...
    def complete_intent_comment_notification(
        self, notification_id: int, *, trace_id: str, message_unique_id: str
    ) -> None:
        now = utc_now()
        with self._write_lock, self.connect() as conn:
    ...
    def defer_intent_comment_notification(
        self,
        notification_id: int,
        *,
        next_attempt_at: str,
    ...
    def recover_intent_comment_notifications(self) -> int:
        now = utc_now()
        with self._write_lock, self.connect() as conn:
            cursor = conn.execute(
                """
    ...
    def intent_comment_notification_status(self) -> dict[str, int]:
        with self.connect() as conn:
            rows = conn.execute(
                """
                SELECT status, COUNT(*) AS count
                FROM intent_comment_notifications GROUP BY status
                """
            ).fetchall()
        counts = {name: 0 for name in ("pending", "sending", "retry", "sent", "failed")}
        counts.update({str(row["status"]): int(row["count"]) for row in rows})
